...
const handler = async (m, { conn, args }) => {
   ...
   let txt = `           *🌴 𝗕𝗼𝘁 - 𝗦𝗰𝗿𝗶𝗽𝘁 ✨*\n\n`;
      ...
      txt += `◦  *Url:* ${json?.html_url || https://github.com/Kimbrly1/KIMRBOT-MD'}\n\n`;
      txt += `         ${json?.forks_count || '-'} Forks · ${json?.stargazers_count || '-'} Stars · ${json?.open_issues_count || '-'} Issues`;
   await conn.sendMessage(m.chat, {text: txt.trim(), mentions: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), contextInfo: {forwardingScore: 9999999, isForwarded: true, mentionedJid: [...txt.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + '@s.whatsapp.net'), "externalAdReply": {"showAdAttribution": true, "containsAutoReply": true, "renderLargerThumbnail": true, "title": global.titulowm2, "containsAutoReply": true, "mediaType": 1, "thumbnail": imagen6, "mediaUrl": `https://github.com/Kimbrly1/KIMRBOT-MD`, "sourceUrl": `https://github.com/Kimbrly1/KIMRBOT-MD`}}}, {quoted: m});
};
handler.help = ['scbot'];
handler.tags = ['info'];
